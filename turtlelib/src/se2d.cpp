...
namespace turtlelib
{
    ...
    Transform2D::Transform2D(Vector2D trans) : x(trans.x), y(trans.y){}

    Transform2D::Transform2D(double radians) : theta(radians){}

    Transform2D::Transform2D(Vector2D trans, double radians) : x(trans.x), y(trans.y), theta(radians){}

    Point2D Transform2D::operator()(Point2D p) const{
        return {p.x*std::cos(theta) - p.y*std::sin(theta) + x,
